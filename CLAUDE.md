...
| 29 | **MTU polling added to completeConfiguration()** | **Pending test** | Added MTU check/wait in `completeConfiguration()` before HELLO: polls `maximumWriteValueLength` up to 10x (200ms intervals, 2s max) until >= 244. Logs MTU at each poll and warns if still too low. Also corrected comment in `PeripheralManager.swift` — CoreBluetooth does NOT fragment `.withoutResponse` writes; they're silently truncated to `maximumWriteValueLength`. Fix in `BlePodComms.swift`. |
...
- **Phase 2 cannula insertion (CURRENT BLOCKER)**: Phase 1 priming succeeds (test #28). Phase 2 fails due to BLE disconnections during `programBasal`. Root cause: OmnipodKit disconnects/reconnects between Phase 1 and Phase 2, while the official app maintains a continuous BLE connection throughout activation. After reconnection, MTU may not have settled (maximumWriteValueLength=20 instead of 244), causing large `.withoutResponse` writes to be silently truncated. MTU polling fix added (test #29) but untested. The broader question of maintaining continuous BLE connection between phases may also need addressing.
