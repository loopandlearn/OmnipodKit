...
- **REVERTED: SetupPodCommand 24-hour vs 12-hour hour format (test #22, 2026-02-16)**: Originally hypothesized that O5 pods expect 12-hour format (0-11) matching Java `Calendar.HOUR` (field 10), and added `% 12` conversion in `BlePodComms.setupPod()`. This hypothesis was incorrect -- O5 pods actually use 24-hour format like DASH/Eros. The error 33 at test #22 was not caused by hour format. The `% 12` conversion has been reverted; all pod types now use 24-hour format uniformly.
