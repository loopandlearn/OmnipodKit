...
class O5LTKExchanger {
    ...
    private func o5sps2_1() throws -> Data {
        ...
        log.info("Compact proof assembled (%d bytes): %{PRIVATE}@", compactProof.count, compactProof.bytes.toHexString())

        // Encrypt with AES-CCM using the derived conf key
        let nonce = keyExchange.getSPSNonce(direction: .write)
        let key = keyExchange.conf
        log.info("Encrypting SPS2.1 with key %{PRIVATE}@ and nonce %{PRIVATE}@", key.bytes.toHexString(), nonce.bytes.toHexString())
        ...
        let encrypted = try aes.encrypt(compactProof.bytes)
    ...
    private func o5validatePodSps2_1(_ msg: MessagePacket) throws {
        ...
        let aes = try AES(key: key.bytes, blockMode: ccm, padding: .noPadding)
        let decryptedPayload = Data(try aes.decrypt(payload.bytes))
        log.info("Decrypted SPS2.1 payload from pod (%d bytes): %{PRIVATE}@", decryptedPayload.count, decryptedPayload.bytes.toHexString())
        ...
        log.info("Pod ECDSA signature: %{PRIVATE}@", podSignature.bytes.toHexString())
        ...
        if decryptedPayload.count >= keyStartOffset + 65 {
            ...
            log.info("Pod cert public key: %{PRIVATE}@", podCertPubKey.bytes.toHexString())
        ...
        if decryptedPayload.count >= keyStartOffset + 130 {
            ...
            log.info("Pod compact proof key 2: %{PRIVATE}@", key2.bytes.toHexString())
        }
        if decryptedPayload.count >= keyStartOffset + 195 {
            let key3 = decryptedPayload.subdata(in: (keyStartOffset + 130)..<(keyStartOffset + 195))
            log.info("Pod compact proof key 3: %{PRIVATE}@", key3.bytes.toHexString())
        }
    }
    ...
    private func o5sps2() throws -> Data {
        ...
        log.info("Generated SPS2 confirmation (%d bytes): %{PRIVATE}@", confirmationData.count, confirmationData.bytes.toHexString())
        ...
        let aes = try AES(key: key.bytes, blockMode: ccm, padding: .noPadding)
        let encrypted = try aes.encrypt(confirmationData.bytes)
        keyExchange.incrementNonce(direction: .write)
    ...
    private func o5validatePodSps2(_ msg: MessagePacket) throws {
        ...
        let payload = try StringLengthPrefixEncoding.parseKeys([O5LTKExchanger.SPS2], msg.payload)[0]
        log.info("Pod SPS2 payload (%d bytes): %{PRIVATE}@", payload.count, payload.bytes.toHexString())
        ...
        let aes = try AES(key: key.bytes, blockMode: ccm, padding: .noPadding)
        let decryptedPayload = Data(try aes.decrypt(payload.bytes))
        log.info("Decrypted pod SPS2 (%d bytes): %{PRIVATE}@", decryptedPayload.count, decryptedPayload.bytes.toHexString())
        keyExchange.incrementNonce(direction: .read)
        ...
        log.info("Pod CMAC confirmation: %{PRIVATE}@", podConf.bytes.toHexString())
        ...
        if podConf == expectedPodConf {
            log.info("Pod SPS2 CMAC confirmation PASSED")
        } else {
            log.info("Pod SPS2 CMAC confirmation MISMATCH - pod may use extended confirmation format")
            // Log both for debugging - don't throw as the exact format is still being determined
        }
    }
    ...
    private func o5validateP0(_ msg: MessagePacket) throws {
        ...
        if payload != O5LTKExchanger.EXPECTED_P0_PAYLOAD {
            throw PodProtocolError.pairingException("Received unexpected P0 payload: \(payload)")
        }
    }
