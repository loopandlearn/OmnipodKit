...
private func podStatusString(status: DetailedStatus) -> String {
    ...
    if status.faultEventCode.faultType != .noFaults {
        ...
        result += String(format: LocalizedString("\n\n⚠️ Critical Pod Fault %1$@ (%2$@)",
            ...
            String(format: "%03u", status.faultEventCode.rawValue), String(format: "0x%02X", status.faultEventCode.rawValue))
        ...
        if let errorEventInfo = status.errorEventInfo {
            result += String(format: LocalizedString("\nFault Event Info: %1$@ (%2$@),",
                ...
                String(format: "%03u", errorEventInfo.rawValue), String(format: "0x%02X", errorEventInfo.rawValue))
